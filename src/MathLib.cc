...
void MathLib::assignRegularPolygon(int num_vertices, float radius, MathLib::Vec2 position_offset, float rotation_offset, std::vector<MathLib::Point2>& buffer) {
  float rad;
  float angle = 360/(num_vertices);
  
  for (int i = 0; i < num_vertices; i++) {
    rad = (angle*i + rotation_offset) / 180 * M_PI;
    MathLib::Point2 point;
